/* ==========================================================
   AyaSwipe — polished reels UX + Tajwīd + mobile typography
   ========================================================== */
(() => {
  ...
  function requestPlayFor(el, reason='io'){
    ...
    setTimeout(async()=>{
      ...
      try{a.load();}catch{} try{await a.play();}catch{} fadeIn(a,150);
    },20);
  }
  function fadeIn(a,ms=150){ try{a.volume=0;}catch{return} let i=0,steps=8; const step=Math.max(10, Math.floor(ms/steps));
  ...
  // Debounced scroll settle → ensure right card is playing
  ...
  async function setMode(nextMode, opts = {}){
    ...
    document.querySelectorAll('#bottombar .btab[data-mode]').forEach(b=>{
      b.toggleAttribute('aria-current', b.dataset.mode === mode);
    });
    syncPillToActive();
  ...
  function renderSurahList(filter){
    ...
    chapters.forEach(c=>{
      ...
      li.addEventListener('click', async ()=>{
        closeDrawerFn();
        // Jump to start of that surah, keep nav selection as-is
        clearFeed();
  ...
  function applyFontPref(){
    ...
    fontBtn.textContent = '📖'; // icon stays same; could swap if you want
  }
  ...
  startBtn.addEventListener('click', async ()=>{
    ...
    gate.style.display='none'; muteBtn.hidden=false; fontBtn.hidden=false;
